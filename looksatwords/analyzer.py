...
class Analyzer(GnewsGatherer):
    ...
    def __init__(self, db_path="data.json", dfs=None, analyzis_level="default"):
    ...
    def build_words_df(self, hud):
        ...
        words = []
        task = hud.add_task(
            "[purple]Analyzer:Building words DataFrame...", total=2 * len(self.df)
        )
    ...
    def preprocess(self, hud):
        ...
        """
        task = hud.add_task(
            "[purple]Analyzer:Preprocessing data...", total=len(self.df)
        )
    ...
    def analyze(self, hud):
        ...
        task = hud.add_task(
            "[purple]Analyzer:Analyzing data...", total=3 * len(to_process)
        )
        for column in to_process:
            hud.update(task, description=f"[purple]Analyzer:Analyzing {column}...")
            apply_preprocessing_and_sentiment_analysis(
                self.df, f"{column}_cleaned", column
            )
            hud.update(task, advance=1)
            hud.update(
                task, description=f"[purple]Analyzer:Counting words in {column}..."
            )
            apply_wordcount(self.df, f"{column}_cleaned", column)
            hud.update(task, advance=1)
            hud.update(
                task, description=f"[purple]Analyzer:Analyzing grammar in {column}..."
            )
...
def apply_sentiment_analysis(df, column_name, prefix):
    ...
    df[f"{prefix}_positive"] = df[f"{prefix}_sentiment"].apply(
        lambda x: float(x["pos"])
    )
    df[f"{prefix}_negative"] = df[f"{prefix}_sentiment"].apply(
        lambda x: float(x["neg"])
    )
    df[f"{prefix}_neutral"] = df[f"{prefix}_sentiment"].apply(lambda x: float(x["neu"]))
    df[f"{prefix}_compound"] = df[f"{prefix}_sentiment"].apply(
        lambda x: float(x["compound"])
    )
...
def apply_preprocessing_and_sentiment_analysis(df, column_name, prefix, hud):
    ...
    sentiment_task = hud.add_task(
        f"[purple]Analyzer:Analyzing sentiment in {prefix}...", total=1
    )
    apply_sentiment_analysis(df, f"{prefix}_cleaned", prefix)
    hud.update(sentiment_task, advance=1)
    wordcount_task = hud.add_task(
        f"[purple]Analyzer:Counting words in {prefix}...", total=1
    )
...
def apply_grammar_analysis(df, column_name, prefix):
    ...
    for pos_group, pos_tags in pos_groups.items():
        df[f"{prefix}_{pos_group.lower()}"] = df[column_name].apply(
            lambda x: len(
                [word for word, pos in pos_tag(word_tokenize(x)) if pos in pos_tags]
            )
        )
