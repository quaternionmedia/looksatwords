...
- **`docs/cli.md` documented four commands that did not exist** — `gather`,
  `analyze`, `visualize`, `version` — and omitted six that did. Rebuilt from the
  real CLI, and `looksatwords/tests/test_cli_reference.py` now fails in either
  direction.
- **The database path was hard-coded**, so anything demonstrating the tool wrote
  into the operator's file. `LOOKSATWORDS_DB` moves it, and the screenshot run
  uses a scratch database. Question 4 above is answered for new runs; the rows
  already in the development database are still mixed.
- **The README's hero image was captured by hand** and regenerated by no
  command. Deleted. Every picture the prose now shows is written by
  `uv run looksatwords screenshots`, and a test pairs both directions.
